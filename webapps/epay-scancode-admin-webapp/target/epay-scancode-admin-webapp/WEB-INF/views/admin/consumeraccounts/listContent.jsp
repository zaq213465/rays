...
<table class="table table-striped table-bordered table-hover" id="sample_6">
						  ...
						  <c:forEach var="item" items="${pager.result}"> 
						   <tr>						   		
								...
								<td class="parent_id_parse" title="userName">${item.shortName}</td>
								<td class="parent_id_parse" title="phone">${item.phone}</td>
								<td class="parent_id_parse" title="shortName">${item.openId}</td>
								<td class="parent_id_parse" title="roleName">${item.balance}</td>
								<td class="parent_id_parse" title="status">${item.status == 0?'已禁用':'已启用'}</td>
							...
							<td>
								...
								<a href="${path}/consumeraccounts/${item.id}"><span class="label label-info">修改</span></a>
			...
			<script type="text/javascript">
						...
						function todetail(id){
							$.ajax({
								   type: "POST",
								   url: "consumeraccounts/detail/"+id,
