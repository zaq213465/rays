...
<table class="table table-striped table-bordered table-hover" id="sample_6">
						  ...
						  <c:forEach var="item" items="${pager.result}"> 
						   <tr>						   		
								<td class="parent_id_parse" title="redNo">${item.redNo}</td>
								<td class="parent_id_parse" title="senderMerchantNo">${item.senderMerchantNo}</td>
								<td class="parent_id_parse" title="receiverMerchantNo">${item.receiverMerchantNo}</td>
								<td class="parent_id_parse" title="redAmount">${item.amount}</td>
								<td class="date_time_parse" title="time">${item.time}</td>
			...
			<script type="text/javascript">
						...
						function todetail(id){
							$.ajax({
								   type: "POST",
								   url: "consumerreddetail/detail/"+id,
