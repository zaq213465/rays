...
//  	  					$("#accountType").val("");
	...
	$("#channelCode").blur(function(){
		...
		var tel =  /^[A-Za-z0-9]{2}$/;
		if(!tel.test($("#channelCode").val())){
			 $("#" + name+ "-error").remove();
			 $this.parent().parent().append('<label id="' + name + '-error" class="error" for="' + name + '">' +'二级域名编号需是数字、字母,且长度只能是两位，不可用！</label>');
		}
		if($("#channelCode").val()!="" && checkChannelCode($("#channelCode").val())){
			 $("#" + name+ "-error").remove();
			 $this.parent().parent().append('<label id="' + name + '-error" class="error" for="' + name + '">' +'二级域名编号已存在，不可用！</label>');
		}
	});
...
//	   	        required: "请输入【备案号】",
		    ...
		    remark: {
	   	   ...
	   	   wxAgencyId1: {
	   		    required: "微信大商户代理商号不能为空"
	   	      },
	   	   wxAgencyId2: {
	   		    required: "微信一户一码代理商号不能为空"
	   	      },
	   	   zfbAgencyId1: {
	   		    required: "支付宝大商户代理商号不能为空"
	   	      },
	   	   zfbAgencyId2: {
	   		    required: "支付宝一户一码代理商号不能为空"
	   	      }
		},
