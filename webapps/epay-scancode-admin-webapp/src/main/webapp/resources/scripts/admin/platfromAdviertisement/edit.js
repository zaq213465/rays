$(function(){
	...
	$("#form").find("#cancle").click(function(){
		...
		window.location.href= baseUrl + "/adminManage/platfromAdviertisement";
	});	
...
    }).init(function () {
		$("#organNo").next().find("input").on('input', function (e) {
			...
			if(organName != ''){
				$.ajax({
					url:baseUrl + "/adminManage/platfromAdviertisement/getorganlist",
	...
	$("#province").change(function(t){
		...
		if(pid != ''){
			$.ajax({
				url:baseUrl + "/adminManage/platfromAdviertisement/getcitylist",
				data:{"pid":pid},
				async: false,
				type:"POST",
				success:function(data)
...
//   				$("#save").attr("disabled","disabled");
   				...
   				$.ajax({
   					url:baseUrl+"/adminManage/platfromAdviertisement/save",
   					data:datajson,
   					type:"POST",
