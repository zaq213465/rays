...
<table class="table table-striped table-bordered table-hover" id="sample_6">
			...
			<script type="text/javascript">
						...
						$(".addQrcode").on("click",function(){
							var updBtn = $(this);
							var  status=updBtn.parent().parent().find("td:eq(5)").text().trim();
							if(status=='禁用'){
								bootbox.alert("该渠道已被禁用，暂时不能分配二维码！");
								return;
							}
							var flag = 0;
						...
						$(".addLiquidationFee").on("click",function(){
							var zfbFee="";var wxsmFee="";var wxgzhFee="";
							var updBtn = $(this);
							...
							var box = '<form class="form-horizontal"><div class="form-body row">'+
										'<div class="form-group col-md-7">'+
											'<label class="control-label col-md-6">支付宝结算手续费(元)</label>'+
											'<div class="col-md-6">'+
												'<input type="text" class="form-control" id="zfbFee" name="zfbFee" value="'+zfbFee+'" placeholder="支付宝结算手续费(元)" >'+
											'</div>'+
										'</div>'+
										'<div class="form-group col-md-7">'+
											'<label class="control-label col-md-6">微信扫码结算手续费(元)</label>'+
											'<div class="col-md-6">'+
												'<input type="text" class="form-control" id="wxsmFee" name="wxsmFee" value="'+wxsmFee+'" placeholder="微信扫码结算手续费(元)" >'+
											'</div>'+
										'</div>'+
										'<div class="form-group col-md-7">'+
											'<label class="control-label col-md-6">微信公众号结算手续费(元)</label>'+
											'<div class="col-md-6">'+
												'<input type="text" class="form-control" id="wxgzhFee" name="wxgzhFee" value="'+wxgzhFee+'" placeholder="微信公众号结算手续费(元)" >'+
											'</div>'+
										'</div>'+
									   '</div></form>';
						    var btn = {
						    		success:{   
								       ...
								       callback: function() {
								    	    var flag = 0;
								    	    var errMsg = '';
								    	    var zfbFee = $('#zfbFee').val();
								    	    var wxsmFee = $('#wxsmFee').val();
								    	    var wxgzhFee = $('#wxgzhFee').val();
								    	   	if(zfbFee == '') {
								    	   		flag++;
								    	   		errMsg += "支付宝结算手续费不可为空！<br>";
								    	   	}else{
								    	   		var number = /^[0-9]+(\.[0-9]+)?$/; 
								    	   		if(!number.test(zfbFee)){
								    	   			flag++;
								    	   			errMsg += "支付宝结算手续费只能输入数字！<br>";
								    	   		}
								    	   		if(zfbFee.length>10){
								    	   			flag++;
								    	   			errMsg += "支付宝结算手续费长度不能超过10！<br>";
								    	   		}
								    	   	}
								    	   	if(wxsmFee == '') {
								    	   		flag++;
								    	   		errMsg += "微信扫码结算手续费不可为空！<br>";
								    	   	}else{
								    	   		var number = /^[0-9]+(\.[0-9]+)?$/; 
								    	   		if(!number.test(wxsmFee)){
								    	   			flag++;
								    	   			errMsg += "微信扫码结算手续费只能输入数字！<br>";
								    	   		}
								    	   		if(wxsmFee.length>10){
								    	   			flag++;
								    	   			errMsg += "微信扫码结算手续费长度不能超过10！<br>";
								    	   		}
								    	   	}
								    	   	if(wxgzhFee == '') {
								    	   		flag++;
								    	   		errMsg += "微信公众号结算手续费不可为空！<br>";
								    	   	}else{
								    	   		var number = /^[0-9]+(\.[0-9]+)?$/; 
								    	   		if(!number.test(wxgzhFee)){
								    	   			flag++;
								    	   			errMsg += "微信公众号结算手续费只能输入数字！<br>";
								    	   		}
								    	   		if(wxgzhFee.length>10){
								    	   			flag++;
								    	   			errMsg += "微信公众号结算手续费长度不能超过10！<br>";
								    	   		}
								    	   	}
								    	   	...
								    	   	if(flag == 0){								    	   		
									    	    if(channelNo != null && channelNo != ''){
									    	    	$.ajax({
														   ...
														   data:{"channelNo":channelNo.text().trim(),"zfbFee":zfbFee,"wxsmFee":wxsmFee,"wxgzhFee":wxgzhFee},
														   success: function(msg){
															   	msg = $.parseJSON(msg);
													 			bootbox.alert(msg.message,function(){
													 				toPage(1);
