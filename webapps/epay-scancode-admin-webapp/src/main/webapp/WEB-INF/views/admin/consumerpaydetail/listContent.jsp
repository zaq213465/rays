...
<table class="table table-striped table-bordered table-hover" id="sample_6">
						  ...
						  <c:forEach var="item" items="${pager.result}"> 
						   <tr>						   		
								...
								<td class="parent_id_parse" title="businessType">${item.businessType}</td>
								<td class="parent_id_parse" title="orderAmt">${item.orderAmt}</td>
								<td class="parent_id_parse" title="remarks">${item.remarks}</td>
								<td class="date_time_parse" >${item.time}</td>
							<td>
								<%-- <a onclick="javascript:todetail('${item.id}');"><span class="label label-default">查看详情</span></a>
								<a href="${path}/consumerpaydetail/${item.id}"><span class="label label-info">修改</span></a> --%>
			...
			<script type="text/javascript">
						...
						function todetail(id){
							$.ajax({
								   type: "POST",
								   url: "consumerpaydetail/detail/"+id,
