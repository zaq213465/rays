...
//悬浮弹窗
function dialogMsg(obj){
	var box = obj.box || "";
	var callFunc = obj.callFunc || function(){};//关闭弹窗执行的函数
	var className = obj.className || "green-haze";//弹窗的默认样式
	var title = obj.title || "悬浮弹窗";//弹窗的头信息
	var btns = obj.btn || {success:{label: "确认",className: "btn-success",callback: null}}
	bootbox.dialog({
		message: box,
		title: title,
		onEscape: callFunc,
...
function ajaxGetData(obj){
	var defaultOptions ={
			ajaxType : "POST",//ajax类型，post、get
			dataJson :  {},//请求参数
			dataParams : {},//请求处理参数
			dataType: 'json',//参数类型
			cache: false,//是否缓存
			ajaxSync: true,//请求数据是否异步
			callFunc: function(){}//无返回时回调函数
	}
	var dataObj = $.extend( true, {},defaultOptions,obj);
	
	var url = prefixUrl(obj.url);//请求的api路径
	var callFunc = dataObj.callFunc;
	$.ajax({
		url: url,
		data: dataObj.dataJson,
		type: dataObj.ajaxType,
		dataType: dataObj.dataType,
		async: dataObj.ajaxSync,
		cache: dataObj.cache,
		success: function (msg) {
			 if(typeof(obj.dataParams) != 'undefined') msg.dataParams = dataObj.dataParams;
		     callFunc(msg);
		}
	});
}
