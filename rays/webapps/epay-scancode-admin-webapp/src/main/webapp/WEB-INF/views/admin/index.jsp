...
<link rel="shortcut icon" type="image/x-icon" href="${path}/resources/images/favicon.ico">
</head>
<body>
	...
	<section class="tab-content" id="ucenterSection">
		<div class="col-md-12">
			...
			<c:if test="${merchantList>0}">
				<h3>您有&nbsp;
					<a href="#" id="/adminManage/merchantinfo" url="${path }/adminManage/merchantinfo?merchantNoCheck=${merchantList}" 
						addtabs="商户基本信息" title="商户基本信息">
						<span class="merchants">${merchantList}</span>
					</a>&nbsp;个商户未审核</h3>
			</c:if>
			</div>
			<div class="welcome">
				<h3>欢迎<span>${currentScanAdminUser.userName }</span>进入运营平台首页.</h3>
				<br/>
				<p>请根据角色进行对应的操作.</p>
			</div>
		</div>	
