...
<%-- 									<td>${item.paraVal}</td> --%>
									<td class="big-word" style="word-break: break-all; max-width: 600px;" val='${item.paraVal}'  >
										<c:choose>  
										    <c:when test="${fn:length(item.paraVal) > 10}">  
										        <c:out value="${fn:substring(item.paraVal, 0, 10)}" /><b>......</b>
										    </c:when>  
										   <c:otherwise>  
							...
							<td>
								<a onclick="javascript:toDetail('${item.id}');"><span class="label label-danger">查看详情</span></a>
